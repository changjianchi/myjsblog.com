function imgShow(imgList,imgShow,number,width,direction,number){
	...
	if(direction == "margin-left"){
		objUl.width(objLi.length*width);
	}else{
		objUl.height(objLi.length*width);
	}
	...
	function proSlideRun(n){
		...
		objImg.siblings("p").stop().animate({opacity : 0}, 500)
		objImg.fadeTo(300,0,function(){
			objImg.attr("src","");
			objImg.fadeTo(300,1,function(){
				objImg.attr("src",objLi.eq(n).children("img").attr("data-img"));
				objImg.siblings("p").stop().animate({opacity : 1}, 500).find(".inp").html(objLi.eq(n).children("img").attr("data-title"));
			});
		})
	};
