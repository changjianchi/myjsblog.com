...
//flss
    ...
    //-----------
    var listP3NavUlLi4 = $('.list-p4-nav ul li');
    var listP3Cen4 =$('.list-p4-cen');
    listP3NavUlLi4.on('touchstart',function(){
        listP3NavUlLi4.removeClass('active');
        $(this).addClass('active');
        listP3Cen4.addClass('none');
        listP3Cen4.eq($(this).index()).removeClass('none');
    });
//联系我们
...
//导航悬浮
    ...
    var navX = $('.nav-x');
    var winset = setInterval(function(){
        var windowXh = $(window).height();
        if(windowHeight - 20 < windowXh){
            navX.css({
                'position':'fixed'
            });
        }else{
