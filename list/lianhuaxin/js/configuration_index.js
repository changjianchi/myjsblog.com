        ...
        "#addNewSetPage",{
            "click":function() {
               ...
               /* var iframe1 = '<iframe src="../html/configuration-solution/configuration_1.html" id="setFormPage1" width="100%" height="488"  frameborder="0" scrolling="yes" marginheight="0" marginwidth="0"></iframe>';*/
                ...
                var curIframe=$(window.parent.document).find('.wjNewIfr-yibiaoban');
                curIframe.attr('src','./html/configuration-solution/configuration_1.html').attr('id','setFormPage1').height('488px');
...
function Linkage3(args){
    $.extend(this,args);
    var _this=this;

    this.Level1Wrap.on('click','ul li', function (evt) {
        _this.Level2Wrap.children('.wj-depsel-content').text(_this.Level2Wrap.find('ul li:eq(0)').text());
        var id=$(this).data('id'),txt=$(this).text();
        ...
        $.ajax({
            ...
            success: function (msg) {
                ...
                if(msg == ''){
                    ...
                    var  data=eval("("+msg+")");
                    for (var i=0 ;i < data.length; i++) {
                        var str='<li data-id="'+data[i].id+'">'+data[i].tname+'</li>';
                        Lev2Ul.append(str);
                    }
                }
            },
